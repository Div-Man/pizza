...
<header>
    <div class="box">
        ...
        <ul>
            ...
            <?php if (!empty($user)) echo '<li><a class="" onclick="return false;" href="">Профиль</a></li>'; ?>
...
<div class="wrap">
    ...
    <div class="order-success profile-order">
        ...
        foreach ($orders as $val) {
            $newOrders[$val->billId][] = $val;
        }
        ?>
        <div class="user-logout">
            <button class="" type="button" onclick="logout()">Выйти</button>
        </div>
        <h2>Заказы</h2>

        <div class="my-orders-wpar">
            <div class="my-orders">
            ...
            <?php foreach ($newOrders as $orderKey => $orders): ?>
                <div class="main-block-order">
                    <div class="my-orders">
                        ...
                        <div>
                         ...
                         <?php 
                            $newOrder = $_COOKIE["orderId"];
                            if($orderKey == $newOrder){
                                echo  '<div style="color: green;">Новый заказ(оплачено)</div>';
                            }
                         ?>
                        ...
                        foreach ($orders as $order) {
                            $allOrderPrice = $allOrderPrice + $order->totalPrice;
                        }
                        ?>

                        <div><?php echo $allOrderPrice; ?> р.</div>
                    ...
                    <div class="detali">
                        <?php
                        foreach ($orders as $order) {
                            echo $order->title . ', цена: ' . $order->price . 'р, количесво: ' . $order->quantity . '<br>';
                        }
                        ?>
